...
pub fn main() {
    ...
    for line in util::read_input_iter("src/day7/input.txt") {
        ...
        hands.push(Hand {cards: hand, bid, value, value_joker, rank:0, rank_joker: 0});
    }
...
fn evaluate_hand(hand: &Vec<char>) -> i32 {
    ...
    let cards_value = card_value(&hand[0]) * 15_i32.pow(4)
                    ...
                    + card_value(&hand[4]);
    // 5 of a kind
    if sorted[0] == sorted[4] { return 6 * TYPE_VALUE + cards_value; }

    // 4 of a kind
    if sorted[0] == sorted[3] || sorted[1] == sorted[4] { return 5 * TYPE_VALUE + cards_value; }

    // full house
    if sorted[0] == sorted[2] && sorted[3] == sorted[4]
    || sorted[2] == sorted[4] && sorted[0] == sorted[1] { return 4 * TYPE_VALUE + cards_value; }

    // 3 of a kind
    if sorted.windows(3).any(|i | i[0] == i[2]) { return 3 * TYPE_VALUE + cards_value; }

    // two pair
    if sorted[0] == sorted[1] && sorted[2] == sorted[3]
      || sorted[0] == sorted[1] && sorted[3] == sorted[4]
      || sorted[1] == sorted[2] && sorted[3] == sorted[4] { return 2 * TYPE_VALUE + cards_value; }

    // pair
    if sorted.windows(2).any(|i| i[0] == i[1]) { return 1 * TYPE_VALUE + cards_value; }
...
fn evaluate_hand_joker(hand: &Vec<char>) -> i32 {
    ...
    if sorted[2] == 'J' // 3 jokers
       ...
       || sorted.windows(4).any(|i| i[0] == i[3]) // 0 jokers
    ...
    if cmp_j(sorted[0], sorted[2]) && cmp_j(sorted[1], sorted[2]) && sorted[3] == sorted[4] // XXXYY
      || sorted[2] == sorted[4] && cmp_j(sorted[0], sorted[1]) // XXYYY
    ...
    if sorted[1] == 'J' // 2 jokers
      ...
      || sorted.windows(3).any(|i | i[0] == i[2]) // 0 jokers
    ...
    if sorted[0] == 'J' // 1 joker
      || sorted.windows(2).any(|i| i[0] == i[1]) // 0 jokers
