...
use geo::{Coord, line_string, LineString, point, Polygon };
use geo::Contains;
#[path = "../util/util.rs"]
mod util;
...
pub fn main() {
    ...
    let mut pipevec = vec![Coord { y: animal_location.0 as f64, x: animal_location.1 as f64 }];
    let mut steps = 0;
    ...
    if check_u {
        ...
        location = (animal_location.0 + 0, animal_location.1 + 1);
        last_move = 'r';
    } else { panic!("No adjacent pipe found");}
    steps += 1;
    pipevec.push(Coord { y: location.0 as f64, x: location.1 as f64 });
    ...
    for row in 0..tiles.shape()[0] {
        for col in 0..tiles.shape()[1] {
            let coord = Coord { x: col as f64, y: row as f64};
...
fn check_adjacent_pipe(tiles: &Array2<char>, direction: char, location: (usize, usize)) -> bool {
    match direction {
        ...
        'd' => {
            if location.0 == tiles.shape()[0] - 1 { return false; }
            let target = tiles[[location.0 + 1, location.1]];
            return target == 'J' || target == '|' || target == 'L' || target == 'S';
        },
        'r' => {
            if location.1 == tiles.shape()[1] -1 { return false; }
            let target = tiles[[location.0, location.1 + 1]];
...
fn move_location(tiles: &Array2<char>, location: (usize, usize), last_move: char) -> ((usize, usize), char) {
    let current = tiles[location];
    let mut check_l = false; let mut check_u = false; let mut check_r = false; let mut check_d = false;
