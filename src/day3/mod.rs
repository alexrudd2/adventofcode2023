            ...
            if c == '*' {
                ...
                if gearbox[[2, 3]].is_digit(10) {
                    if gearbox[[2, 2]].is_digit(10) {
                        found_numbers[6] =
                            parse_gear_label_right(gearbox.slice(s![2, 1..5]).to_vec());
                    } else {
                        found_numbers[6] =
                            parse_gear_label_right(gearbox.slice(s![2, 2..6]).to_vec());
                    }
                }
...
fn is_adjacent_to_symbol(
    ...
    // up-left
    if row > 0 && col > 0 && is_symbol(array[[row - 1, col - 1]]) {
        return true;
    }
    // up
    if row > 0 && is_symbol(array[[row - 1, col]]) {
        return true;
    }
    // up-right
    if row > 0 && col < cols - 1 && is_symbol(array[[row - 1, col + 1]]) {
        return true;
    }
    // left
    if col > 0 && is_symbol(array[[row, col - 1]]) {
        return true;
    }
    // right
    if col < cols - 1 && is_symbol(array[[row, col + 1]]) {
        return true;
    }
    // down-left
    if row < rows - 1 && col > 0 && is_symbol(array[[row + 1, col - 1]]) {
        return true;
    }
    // down
    if row < rows - 1 && is_symbol(array[[row + 1, col]]) {
        return true;
    }
    // down-right
    if row < rows - 1 && col < cols - 1 && is_symbol(array[[row + 1, col + 1]]) {
        return true;
    }
    return false;
}
